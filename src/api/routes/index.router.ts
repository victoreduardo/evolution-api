...
const metricsIPWhitelist = (req: Request, res: Response, next: NextFunction) => {
  ...
  const clientIPs = [req.ip, req.connection.remoteAddress, req.socket.remoteAddress, req.headers['x-forwarded-for']].filter((ip) => ip !== undefined);

  if (allowedIPs.filter(ip => clientIPs.includes(ip)) === 0) {
    return res.status(403).send(`Forbidden: IP not allowed: ${clientIPs}`);
  }
